...
object PatternMaching extends App {
  ...
  * */
  ...
  val animal: Animal = Dog("Terre Nova")
  ...
  *  Prod(Sum(Number(3), Number(1)), Number(3)) = (2 + 1) *3
  ...
  * */
  ...
  def show(e: Expr): String = e match {
    ...
    case Prod(e1, e2) => {
      def maybeShowParen(exp: Expr) = exp match {
        case Prod(_, _) => show(exp)
        case Number(_) => show(exp)
        case _ => "(" + show(exp) + ")"
      }

      maybeShowParen(e1) + " * " + maybeShowParen(e2)
    }
